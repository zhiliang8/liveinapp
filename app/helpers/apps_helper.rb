  ...
  def render_stars(app, modify=false)
    ...
    App::RATE_DIMENSIONS.each do|k, v|
      ...
      stars_status.each do |k1, v1|
        ...
        input_html << %(<input name="star_#{k}" type="radio" class="star required" #{disabled="disabled" unless modify} value="#{k1}" title="#{v1}" #{checked="checked" if checked}/>)
      end
